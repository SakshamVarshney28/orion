...
const DEBOUNCE_DELAY = 600; // wait a bit longer for user to pause typing
let currentAbortController: AbortController | null = null;
let lastSuggestionAt = 0;
const MIN_SUGGESTION_INTERVAL = 1500; // ms between actual API calls
    ...
    class {
      ...
      triggerSuggestion(view: EditorView) {
        ...
        debounceTimer = window.setTimeout(async () => {
          const now = Date.now();
          // Hard rate-limit: skip if we've asked for a suggestion very recently.
          if (now - lastSuggestionAt < MIN_SUGGESTION_INTERVAL) {
